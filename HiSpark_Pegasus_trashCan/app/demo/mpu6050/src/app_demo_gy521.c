...
hi_u8 Gy521_Write_Len(hi_u8 addr, hi_u8 reg, hi_u8 len, hi_u8 *buf)
{
    ...
    if (ret == HI_ERR_I2C_INVALID_PARAMETER) {
        GYROSCOPE_LOG_ERROR("i2c invalid parameter 0x%x\r\n", ret);
        return HI_ERR_FAILURE;
    } 
    if (ret == HI_ERR_I2C_WAIT_SEM_FAIL) {
        GYROSCOPE_LOG_ERROR("i2c wait sem failed 0x%x\r\n", ret);
        HI_ERR_FAILURE;
    }
    return ret;
}
...
hi_u8 GY521_Read_Len(hi_u8 addr, hi_u8 reg, hi_u8 len, hi_u8 *data)
{
    ...
    ret = hi_i2c_write(id, addr<<1|MPU_ADDR_WRITE, &i2c_data);
    if (ret == HI_ERR_I2C_INVALID_PARAMETER) {
        GYROSCOPE_LOG_ERROR("GY521_Read_Len : i2c invalid parameter 0x%x\r\n", ret);
        return HI_ERR_FAILURE;
    } 
    /*read data*/
    ...
    if (ret == HI_ERR_I2C_WAIT_SEM_FAIL) {
        ...
        GYROSCOPE_LOG_ERROR("i2c not init 0x%x\r\n", ret);
...
hi_void *gy521_mpu6050(hi_void* param)
{
	...
	while(1)
	{
        ...
        GYROSCOPE_LOG_INFO("Pitch : %.02f Roll : %.02f \n",Pitch,Roll);
        if(((((Pitch > -2.2) && (Pitch <= 0)) && ((Roll > -89) && (Roll <= -30))) || (((Pitch > 0) && (Pitch <= 5)) && ((Roll >= -10) && (Roll <= 11)))) && (flag_demped == 0)) {//正面
            if (wifi_status == HI_WIFI_EVT_CONNECTED) {
                GYROSCOPE_LOG_INFO("publish SmartCan positive demped");
                iot_publish_tarsh_is_dumped("true");
                flag_demped = 1; 
            } else {
                GYROSCOPE_LOG_INFO("publish SmartCan positive demped, but huawei cloud is not connected");
            }
        } else if (((Pitch > 15) && (Pitch <= 90)) && ((Roll > -180) && (Roll <= -90)) && (flag_demped == 0)) { //右侧面
            if (wifi_status == HI_WIFI_EVT_CONNECTED) {
                GYROSCOPE_LOG_INFO("publish SmartCan Right side demped");
                iot_publish_tarsh_is_dumped("true");
                flag_demped = 1; 
            } else {
                GYROSCOPE_LOG_INFO("publish SmartCan Right side demped, but huawei cloud is not connected");
            }
        } else if (((Pitch > -4.00) && (Pitch <= -1.4)) && ((Roll > -179) && (Roll <= -100)) && (flag_demped == 0)) { //后面 
            if (wifi_status == HI_WIFI_EVT_CONNECTED) {
                GYROSCOPE_LOG_INFO("publish SmartCan behind side demped");
                iot_publish_tarsh_is_dumped("true");
                flag_demped = 1; 
            } else {
                GYROSCOPE_LOG_INFO("publish SmartCan behind side demped, but huawei cloud is not connected");
            }
        } else if (((Pitch > -90) && (Pitch <= -5)) && ((Roll > -90) && (Roll <= 90)) && (flag_demped == 0)) { //左侧面
            if (wifi_status == HI_WIFI_EVT_CONNECTED) {
                GYROSCOPE_LOG_INFO("publish SmartCan left side demped");
                iot_publish_tarsh_is_dumped("true");
                flag_demped = 1; 
            } else {
                GYROSCOPE_LOG_INFO("publish SmartCan left side demped, but huawei cloud is not connected");
            }
        } else if (((Pitch > -4.9) && (Pitch <= -3.0)) && ((Roll > -89.99) && (Roll <= -85.99)) && (flag_demped == 1)) {
            if (wifi_status == HI_WIFI_EVT_CONNECTED) {
                printf("publish not demped\r\n");
                iot_publish_tarsh_is_dumped("true");
                flag_demped = 0; 
            } else {
                GYROSCOPE_LOG_INFO("publish not demped, but huawei cloud is not connected");
            }
        }
		hi_sleep(500);
	}
...
int app_demo_gy521(void)
{
    ...
    if (ret != HI_ERR_SUCCESS) {
       GYROSCOPE_LOG_ERROR("GY521 TASK CTREATE FAILED");
    }
